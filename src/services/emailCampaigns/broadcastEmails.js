...
const GET_BROADCAST_EMAIL_TEMPLATE_DETAIL_ROUTE = 'broadcast_email/get_template_content/{template_id}';
const GET_BROADCAST_EMAIL_SEARCH_USER_ROUTE = 'broadcast_email/search-user?fields=first_name,last_name,username&search=';
...
export function getBroadcastEmailTemplateDetailApi(template_id, callback, failCallback) {
  return request(GET_BROADCAST_EMAIL_TEMPLATE_DETAIL_ROUTE.replace("{template_id}", template_id), {
    method: 'GET',
    headers: { Authorization: `Bearer ${getToken()}` }
  }).then(({ data, response }) => {
    if (response) {
