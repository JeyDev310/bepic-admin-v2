...
const PAYOUT_CYCLE_EXPORT = 'report/payout/{id}/export_csv'
...
export function getPayoutCycleExportApi(id, callback, failCallback) {
  return request(PAYOUT_CYCLE_EXPORT.replace('{id}', id), {
    method: 'GET',
    headers: { Authorization: `Bearer ${getToken()}` },
  }).then(({ data, response }) => {
    if (response) {
