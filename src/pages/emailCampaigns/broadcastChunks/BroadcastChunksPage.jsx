...
import { getBroadcastChunksApi, resendBroadcastChunkApi } from '@/services/emailCampaigns/broadcastEmails';
...
const BroadcastChunksPage = (props) => {
  ...
  const columns = [
    ...
    },
    {
      title: 'Sent at',
      key: 'sent_at',
      render: (_, record) => <p className="mb-0">{record.sent_date && asDateTime(record.sent_date)}</p>,
    },
    {
      title: 'Status',
      key: 'status',
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <Space size={10}>
          <OutlineBtn onClick={() => handleClick(record.id)}> Resend </OutlineBtn>
          </Space>
        );
      },
    },
        ...
        <Row>
          <Col xs={24}>
            <TablePanel
              ...
              title={
                t("pages.emailCampaigns.broadcastChunks", "Broadcast Chunks")
              }
              columns={columns}
            />
          </Col>
