...
const SystemEmailsPage = () => {
  ...
  const handleSearch = () => {
    setIsLoading(true);
    getSystemEmailsApi({}, onGetSystemEmails, onFailSystemEmails);
  };

  const handleTest = (system_email_value) => {
    setTestEmailValue(system_email_value);
  ...
  const columns = [
    ...
    },
    {
      title: 'Updated By',
      key: 'updated_by',      
      render: (_, record) => (record.updated_admin && 
        <p className="mb-0">
    ...
    },
    {
      title: 'Updated At',
      key: 'updated_at',
      render: (_, record) => <p className="mb-0">{asDate(record.updated_at)}</p>,
    },
    {
      title: 'Status',
      key: 'status',
...

    {
      ...
      render: (_, data) => (
        <span className="d-flex">
          ...
          <OutlineBtn className="ml-10" onClick={() => handleTest(data.value)}>Test</OutlineBtn>
        </span>
      ),
    },
  ];

  const onFinishTest = (res) => {
  ...
  return (
    <PageContainer>      
        ...
        <Row>
          <Col xs={24}>
            <TablePanel
              data={tableData}
              title={
                t("pages.emailCampaigns.systemEmails","System Emails")
              }
              columns={columns}
              loading={isLoading}
            />
      ...
      {isModalVisible && <TestSystemEmailModal 
