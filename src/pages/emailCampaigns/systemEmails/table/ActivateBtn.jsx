...
export default function ActivateBtn(props) {
  ...
  const onActiveSystemEmail = () => {
    ...
    notification.success({
      ...
      description: t({
        id: 'pages.systemEmail.activateSuccess',
        defaultMessage: 'Email activation changed successfully',
      }),
    });
    props.handleSearch();
  };
  ...
  const handleActivate = () => {
    setIsUpdating(true);
    const status = varKey('systemEmail.status', props.data.status) == 'active' ? 
      varValue('systemEmail.status', 'inactive') : varValue('systemEmail.status','active');
